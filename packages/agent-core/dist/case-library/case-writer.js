...
export class CaseWriter {
    ...
    async extractViaLLM(investigation, conclusion) {
        ...
        try {
            const response = await this.llm.complete([
                {
                    role: 'system',
                    content: 'You are an SRE knowledge-base engineer. ' +
    ...
    buildExtractionPrompt(investigation, conclusion) {
        ...
        return JSON.stringify({
            instruction: 'Extract a generalised, reusable case record from the investigation below. ' +
    ...
    parseExtraction(raw) {
        ...
        const rootCause = typeof obj.rootCause === 'string' && obj.rootCause.trim() ? obj.rootCause.trim() : null;
        const resolution = typeof obj.resolution === 'string' && obj.resolution.trim() ? obj.resolution.trim() : null;
...
//# sourceMappingURL=case-writer.js.map