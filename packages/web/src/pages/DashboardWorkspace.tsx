...
export default function DashboardWorkspace() {
  ...
  // Listen for `dashboard:refresh-panels` events emitted by
  // ChangeProposalCard on accept / inline panel edits. Re-fetch dashboard
  // so the just-applied change becomes visible without a manual refresh.
  useEffect(() => {
    const handler = (e: Event) => {
      const detail = (e as CustomEvent<{ dashboardId?: string }>).detail;
      // A bare `dashboard:refresh-panels` means "re-query panel data".
      // Dashboard JSON reload is reserved for mutation flows that include
      // the affected dashboard id (accepting a proposal, adding panels, etc.).
      if (!detail?.dashboardId || detail.dashboardId !== id) return;
      void loadDashboard();
    };
    window.addEventListener('dashboard:refresh-panels', handler);
    return () => window.removeEventListener('dashboard:refresh-panels', handler);
  }, [id, loadDashboard]);

  // Chat / SSE — events / sendMessage / stopGeneration are now handled
  // by the global Layout-mounted ChatPanel (via useGlobalChat). The
  // dashboard hook is still the authoritative source for live SSE-driven
  // panel/variable mutations and investigation reports.
  const {
