/**
 * PostgresGithubAppConfigRepository — Postgres sibling of the SQLite
 * implementation. Same encryption wire format, same row decode.
 */
...
function rowToConfig(r: Row): GithubAppConfig {
  ...
  return {
    orgId: r.org_id,
    appId: typeof r.app_id === 'string' ? Number(r.app_id) : r.app_id,
    slug: r.slug,
...
export class PostgresGithubAppConfigRepository implements IGithubAppConfigRepository {
  ...
  async delete(orgId: string): Promise<boolean> {
    ...
    await this.db.run(sql`DELETE FROM github_app_config WHERE org_id = ${orgId}`);
    return true;
  }
}
