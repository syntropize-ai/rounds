...
export function createQueryRouter() {
  ...
  router.post('/range', authMiddleware, async (req, res) => {
    const { query, step = '30s', datasourceId, environment, cluster } = req.body;
    if (!query) {
    ...
    const endDate = req.body.end ? new Date(req.body.end) : new Date();
    const startDate = req.body.start ? new Date(req.body.start) : new Date(endDate.getTime() - 30 * 60 * 1000);
    try {
      const client = new PrometheusClient(buildClientConfig(ds));
  ...
  // GET /api/query/metadata?match={pattern}&datasourceId=xxx&environment=prod&cluster=cluster-a
  // Returns metric names matching the pattern - used by AI generator to discover metrics
  router.get('/metadata', authMiddleware, async (req, res) => {
    ...
    try {
      ...
      if (!fetchRes.ok) {
        res.status(502).json({ error: { code: 'PROMETHEUS_ERROR', message: `Prometheus HTTP ${fetchRes.status}` } });
        return;
      }
  ...
  // GET /api/query/labels?metric={name}&datasourceId=xxx&environment=prod&cluster=cluster-a
  ...
  router.post('/batch', authMiddleware, async (req, res) => {
    const { queries, step = '30s', datasourceId, environment, cluster } = req.body;
