        ...
        const defaults = [
            ...
            },
            {
                ...
                mappings: [
                    {
                        backend: 'prometheus',
                        queryTemplate: 'avg(rate(container_cpu_usage_seconds_total{namespace="{{namespace}}",pod=~"{{service}}.*"}[{{window}}])) / avg(kube_pod_container_resource_limits{namespace="{{namespace}}",pod=~"{{service}}.*",resource="cpu"})',
                        labelConstraints: [
