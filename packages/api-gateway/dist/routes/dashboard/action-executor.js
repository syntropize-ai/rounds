    ...
    switch (action.type) {
      ...
      case 'rearrange': {
        const panels = dashboard.panels.map((p) => {
          ...
          return layout ? { ...p, row: layout.row, col: layout.col, width: layout.width, height: layout.height } : p;
        });
        await this.store.updatePanels(dashboardId, panels);
        for (const layout of action.layout) {
          this.sendEvent({
            ...
            patch: { row: layout.row, col: layout.col, width: layout.width, height: layout.height },
          });
        }
        break;
      }
      ...
      case 'add_variable': {
        ...
        this.sendEvent({ type: 'variable_created', variable: action.variable });
        break;
      }
