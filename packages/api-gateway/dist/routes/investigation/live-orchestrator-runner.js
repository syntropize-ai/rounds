...
export class LiveOrchestratorRunner {
    ...
    async execute(input) {
        ...
        try {
            ...
            const { conclusion, hypotheses } = await this.analyzeEvidence(gateway, model, question, allEvidence, investigationId);
            ...
            await this.feed.add('investigation_complete', question.length > 50 ? `${question.slice(0, 57)}...` : question, conclusion.summary, conclusion.confidence >= 0.7 ? 'medium' : 'low', investigationId);
        }
        catch (err) {
    ...
    async analyzeEvidence(gateway, model, question, evidence, investigationId) {
