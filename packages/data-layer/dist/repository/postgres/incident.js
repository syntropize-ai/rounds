...
export class PostgresIncidentRepository {
  ...
  async findById(id) {
    ...
    if (!row)
      return undefined;
    const timeline = await this.db.select().from(incidentTimeline).where(eq(incidentTimeline.incidentId, id));
  ...
  async findByService(serviceId, tenantId) {
    const rows = await this.db.select().from(incidents).where(isNull(incidents.archivedAt));
    return rows
      .filter((r) => r.services.includes(serviceId) && (tenantId === undefined || r.tenantId === tenantId))
