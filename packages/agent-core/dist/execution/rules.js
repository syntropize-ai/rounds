...
const VALID_POLICY_TAGS = new Set(['suggest', 'approve_required', 'deny']);
const VALID_RISKS = new Set(['low', 'medium', 'high']);
// ---------------------------------------------------------------------------
// Core LLM classification function
// ---------------------------------------------------------------------------
export async function classifyAndRecommendActions(hypotheses, entity, gateway, model) {
    const activeHypotheses = hypotheses.filter((h) => h.hypothesis.status !== 'refuted');
    ...
    try {
        const response = await gateway.complete([
            {
                role: 'system',
                content: 'You are an expert SRE remediation advisor. Given investigation hypotheses and supporting evidence, recommend specific remediation actions. Always respond with valid JSON matching the required schema.',
            },
...
// Prompt builder
// ---------------------------------------------------------------------------
function buildClassificationPrompt(hypotheses, entity) {
    return JSON.stringify({
        instruction: 'Analyze these investigation hypotheses and recommend specific remediation actions for the affected entity. ' +
            ...
            actions: [
                {
                    ...
                    params: 'object with relevant parameters (e.g. service, scaleDirection, severity)',
                    rationale: 'explanation of why this action addresses the root cause',
...
function parseClassificationResponse(raw, hypotheses, entity) {
    ...
    const defaultInvestigationId = hypotheses[0]?.hypothesis.investigationId ?? '';
    const seen = new Set();
    const actions = [];
    for (const item of rawActions) {
        ...
        if (seen.has(type))
            continue;
        seen.add(type);
        const policyTag = VALID_POLICY_TAGS.has(rec.policyTag)
            ? rec.policyTag
            : 'suggest';
        const risk = VALID_RISKS.has(rec.risk)
            ? rec.risk
            : 'low';
        const params = typeof rec.params === 'object' && rec.params !== null
            ? rec.params
            : { service: entity };
        actions.push({
            id: nextId(),
...
export const rollbackRule = makeLegacyRule('rollback-on-deploy');
export const scaleRule = makeLegacyRule('scale-on-saturation');
export const configReviewRule = makeLegacyRule('review-config-change');
export const genericTicketRule = makeLegacyRule('ticket-for-investigation');
export const criticalNotifyRule = makeLegacyRule('notify-on-critical');
