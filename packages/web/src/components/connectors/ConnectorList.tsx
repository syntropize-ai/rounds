...
export function ConnectorList({
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex-1 overflow-y-auto px-2 py-3 space-y-4">
        ...
        {!loading && connectors.length === 0 && (
          <p className="px-2 text-xs text-[var(--color-on-surface-variant)]">
            No connectors yet. Use “+ Add connector” below to create one.
          </p>
        )}
      ...
      <div className="border-t border-[var(--color-outline-variant)]/30 p-2">
        <button
          type="button"
          disabled={!canWrite}
          aria-pressed={creating}
          onClick={onAddClick}
          className={`flex w-full items-center justify-center gap-2 rounded-md border border-dashed px-2 py-2 text-sm transition-colors disabled:opacity-50 ${
            creating
              ? 'border-[var(--color-primary)] text-[var(--color-primary)] bg-[var(--color-primary)]/10'
              : 'border-[var(--color-outline-variant)] text-[var(--color-on-surface-variant)] hover:bg-[var(--color-surface-high)]/60'
          }`}
        >
