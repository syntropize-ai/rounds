...
import { postMortemStore as defaultPmStore } from './post-mortem-store.js';
...
export function createIncidentRouter(store = incidentStore, extras = {}) {
    ...
    const pmStore = extras.pmStore ?? defaultPmStore;
    ...
    router.post('/:id/investigations', requirePermission('incident:create'), async (req, res, next) => {
        try {
            ...
            if (!investigationId || typeof investigationId !== 'string') {
                res.status(400).json({ code: 'INVALID_INPUT', message: 'investigationId is required' });
                return;
            }
    ...
    router.post('/:id/post-mortem', requirePermission('incident:read'), async (req, res, next) => {
        try {
            ...
            if (!generator) {
                res.status(503).json({ code: 'SERVICE_UNAVAILABLE', message: 'Post-mortem generator is not configured' });
                return;
            }
            ...
            const investigations = incident.investigationIds
                ...
                hypotheses: inv.hypotheses.map(h => ({ description: h.description, confidence: h.confidence ?? 0 })),
                evidence: inv.evidence.map(e => ({ type: e.type, summary: e.summary })),
            }));
            const input = {
                id: incident.id,
                ...
                investigations,
                executionResults: Array.isArray(body?.executionResults)
                    ? body.executionResults
                    : [],
                verificationOutcomes: Array.isArray(body?.verificationOutcomes)
                    ? body.verificationOutcomes
                    : [],
            };
