...
export class ResearchAgent {
  ...
  constructor(gateway, model, sendEvent) {
  ...
  async buildSearchQuery(topic) {
    try {
      const resp = await this.gateway.complete([
        {
          role: 'system',
          content: "You are a search query optimizer. Convert the user's input into an effective English web search query for finding Prometheus monitoring metrics and best practices for the relevant technology. Return ONLY the query string, nothing else.",
        },
        { role: 'user', content: topic },
      ], { model: this.model, maxTokens: 64, temperature: 0 });
  ...
  async extractKnowledge(topic, snippets) {
...
Return JSON: { keyMetrics: [...], metricPrefixes: [...], monitoringApproach: "...", bestPractices: [...], panelSuggestions: [...] }`,
    ...
    try {
      ...
      console.log(`[ResearchAgent] LLM extraction raw (first 500): ${cleaned.slice(0, 500)}`);
      ...
      return {
        ...
        monitoringApproach: typeof parsed.monitoringApproach === 'string'
          ? parsed.monitoringApproach
          : FALLBACK_KNOWLEDGE.monitoringApproach,
