 ...
 *
 * Lives in agent-core (vs. api-gateway) so agent tool handlers can fire
 * panel-event rows directly without depending on the gateway package. The
 * gateway re-exports this module so the Express dashboard route keeps its
...
/**
 * Structural mirror of `@agentic-obs/data-layer`'s panel-event types. We
 * intentionally do NOT import from data-layer here: agent-core has only a
 * devDependency on data-layer, and pulling it in at runtime would invert the
 * package layering. The api-gateway side re-exports this module and supplies
 * the concrete repository instance, which structurally satisfies this shape.
 */
...
export interface IPanelEventRepository {
  record(input: PanelEventInput): Promise<{ id: string }>;
}
