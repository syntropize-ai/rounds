...
> 说明：本文件内容直接来自你提供的正文粘贴稿。由于原帖中的图片无法复制，这里仅保留文字内容，并在文中缺图处加上占位说明，后续你发图我可以继续补进去。
...
后面第 6 节会提到 `| head -30` 这种手动截断，RTK 干的就是这件事，只是覆盖面更广，不用每条命令自己加，项目开源在 GitHub。
...
`[图片缺失：这里原文有一张解释 AskUserQuestion 设计更稳定的图]`
