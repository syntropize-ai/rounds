// Dashboard API router - all /dashboards endpoints
import { Router } from 'express';
import { randomUUID } from 'crypto';
import { authMiddleware } from '../../middleware/auth.js';
import { requirePermission } from '../../middleware/rbac.js';
import { defaultDashboardStore } from './store.js';
...
export function createDashboardRouter(deps = {}) {
  const store = deps.store ?? defaultDashboardStore;
  ...
  // IMPORTANT: these routes must be defined before /:id to avoid Express treating "investigations" as an id param
  ...
  router.get('/:id', requirePermission('dashboard:read'), async (req, res, next) => {
    try {
      const d = await store.findById(req.params['id'] ?? '');
      if (!d) {
  ...
  router.post('/:id/chat', requirePermission('dashboard:write'), async (req, res) => {
    const body = req.body;
    if (!body.message || typeof body.message !== 'string' || !body.message.trim()) {
      res.status(400).json({ code: 'INVALID_INPUT', message: 'message is required and must be a non-empty string' });
      return;
    }
    await handleChatMessage(req, res, req.params['id'] ?? '', body.message.trim(), store, conversationStore);
  });
  ...
  router.post('/:id/variables/resolve', requirePermission('dashboard:read'), async (req, res, next) => {
    try {
      ...
      const promDs = config.datasources.find((d) => d.id === datasourceId && (d.type === 'prometheus' || d.type === 'victoria-metrics'));
      let prometheusUrl = '';
      const headers = {};
      if (promDs) {
