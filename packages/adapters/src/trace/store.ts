...
import type { Span, Trace, TraceQuery, WaterfallNode, SpanStatus } from './types.js';

export interface TraceStoreConfig {
  /** Max spans to keep per trace (excess spans are dropped, trace.truncated = true) */
...
export class TraceStore {
  ...
  query(q: TraceQuery): Trace[] {
    ...
    for (const trace of this.traces.values()) {
      if (!this.matchesQuery(trace, q)) continue;
      results.push(trace);
    }
  ...
  private upsertTrace(traceId: string, spans: Span[]): void {
    ...
    const endMs = Math.max(
      ...limited.map((s) => new Date(s.startTime).getTime() + s.durationMs),
    );
  ...
  private deriveStatus(spans: Span[]): SpanStatus {
    if (spans.some((s) => s.status === 'error')) return 'error';
    if (spans.some((s) => s.status === 'ok')) return 'ok';
    return 'unset';
  }

  private matchesQuery(trace: Trace, q: TraceQuery): boolean {
    const traceStart = new Date(trace.startTime);
    if (traceStart < q.startTime || traceStart > q.endTime) return false;
  ...
  private buildNode(
    ...
    const children = childSpans.map((child) =>
      this.buildNode(child, childrenByParent, depth + 1, rootStartMs),
    );
