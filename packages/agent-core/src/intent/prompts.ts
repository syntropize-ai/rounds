export const INTENT_SYSTEM_PROMPT = `You are an observability intent parser.
Your job is to convert a natural-language question about a software system into a structured JSON intent object.
Output ONLY valid JSON - no markdown, no prose, no code fences.

## JSON schema for this schema
{
  "taskType": one of "explain_latency" | "explain_errors" | "check_health" | "compare_baseline" | "investigate_change" | "general_query",
  "entity": string (service, endpoint, or component name; use "unknown" if not named),
...
- taskType mapping:
  ...
  - "compare_baseline"  -> questions about regressions, before/after, comparison, degradation
...
export function buildPromptMessage(message: string, now: string): string {
