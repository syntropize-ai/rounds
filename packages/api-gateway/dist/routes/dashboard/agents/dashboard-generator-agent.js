...
export class DashboardGeneratorAgent {
  ...
  async generate(input, onGroupComplete) {
    ...
    const groupResults = await Promise.all(plan.groups.map((group, i) =>
      this.generateAndCriticLoop(group, input, researchResult, discoveryResult, groupStartRows[i])
        .then(async (panels) => {
  ...
  // LLM-based metric selection (no rule-based pref.)
  async selectRelevantMetrics(goal, allMetrics) {
    ...
    try {
      ...
      const resp = await this.deps.gateway.complete([
        {
          role: 'system',
          content: `You are a Prometheus expert. Given a monitoring goal and a list of available metric names, select the ones that are most relevant for building a dashboard.
Select 10-25 of the relevant metric names (exact strings from the list).
...
ONLY return the JSON array, nothing else.`,
      ...
      ], { model: this.deps.model, maxTokens: 4096, temperature: 0, responseFormat: 'json' });
  ...
  async plan(input, research, discovery) {
    const researchContext = research
      ...
      : '';
    const metricsContext = discovery && discovery.metrics.length > 0
      ? `\n## Available Metrics (from user's Prometheus - supplementary)\n${discovery.metrics.slice(0, 80).join('\n')}\n\nThese metrics exist in the cluster.
Prefer them when relevant, but also include important standard metrics that may not be in this list.\nUse your knowledge of standard Prometheus metric naming for this technology.`
      : '';
...
5. Use diverse visualization types:
- stat: single KPI numbers
- time_series: trends over time
- gauge: current percentage/ratio values
- bar: top-N rankings or categorical comparisons
- table: detailed multi-column instant data
- pie: proportional breakdown
- histogram: latency distribution from _bucket metrics
- status_timeline: discrete up/down state over time
6. Layout uses a 12-column grid:
- stats width=3, height=2
- time_series width=6 or 12, height=3
...
A good dashboard uses a mix of visualization types. For 12-18 panels, aim for roughly:
- 3-4 time series for key KPIs
- 1-2 stat panels
- 1-2 gauge for utilization percentages
- 1-2 bar for top-N comparisons
- 1 pie for proportional breakdowns
- 1 status_timeline for health/availability
- 1 histogram for latency distribution (if _bucket metrics exist)
Do NOT make everything time_series or stat. Choose the visualization that best fits the data.
...
## Output Format (JSON)
{
  ...
  "variables": [
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        ], { model: this.deps.model, maxTokens: 8192, temperature: 0.2, responseFormat: 'json' });
        const cleaned = resp.content.replace(/```json\n?/g, '').replace(/```/g, '').trim();
        const parsed = JSON.parse(cleaned);
        return {
          title: parsed.title,
  ...
  async generateAndCriticLoop(group, input, research, discovery, startRow) {
    ...
    for (let round = 0; round <= MAX_CRITIC_ROUNDS; round++) {
      this.deps.sendEvent?.({
        type: 'tool_call',
        tool: 'generator_group',
        args: { group: group.label, round },
        displayText: `Generating "${group.label}" (round ${round + 1}${feedback.length ? `, revise ${feedback.length} issues` : ''})`,
      });
      ...
      this.deps.sendEvent?.({
        ...
        displayText: `Built ${group.label}: ${rawPanels.length} panels`,
      });
  ...
  async generateGroup(group, input, research, discovery, startRow, criticFeedback) {
    const researchContext = research?.keyMetrics.length
      ? `Use these as reference alongside your own knowledge:\n${research.keyMetrics.join(', ')}`
      : '';
    const metricsSection = discovery && discovery.metrics?.length
      ? `Available metrics (from Prometheus - supplementary):\n${discovery.metrics.slice(0, 15).join('\n')}`
      : '';
    const labelsSection = discovery?.labelByMetric
      ? Object.entries(discovery.labelByMetric).slice(0, 15).map(([k, v]) => `- ${k}: ${(v ?? []).join(', ')}`).join('\n')
      : '';
...
${JSON.stringify(group.panelSpecs)}
...
- For multi-series comparison panels add "instant": true to the query
...
approved = true if overallScore >= 8 AND no severity=error issues.`;
    ...
    try {
      ...
      ], { model: this.deps.model, maxTokens: 2048, temperature: 0, responseFormat: 'json' });
      const cleaned = resp.content.replace(/```json\n?/g, '').replace(/```/g, '').trim();
      const parsed = JSON.parse(cleaned);
      return {
    ...
    for (const panel of panels) {
      ...
      if (allValid) {
        ...
        this.deps.sendEvent?.({
          type: 'tool_result',
          tool: 'validate_query',
          summary: `${panel.title} - query validation failed`,
          success: false,
        });
      }
    }
  ...
  async queryPrometheus(expr) {
    if (!this.deps.prometheusUrl)
      return false;
    try {
      const url = `${this.deps.prometheusUrl}/api/v1/query?query=${encodeURIComponent(expr)}`;
      const resp = await fetch(url, { headers: this.deps.prometheusHeaders });
      return resp.ok;
    }
    catch {
      return false;
  ...
  detectVariables(panels, input, discovery) {
    ...
    for (const panel of panels) {
      for (const q of panel.queries ?? []) {
        for (const m of String(q.expr ?? '').matchAll(/\$([a-zA-Z_][a-zA-Z0-9_]*)/g)) {
    ...
    for (const varName of referencedVars) {
      ...
      let sourceMetric;
      if (discovery) {
        for (const [metric, labels] of Object.entries(discovery.labelByMetric ?? {})) {
