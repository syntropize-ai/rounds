/**
 * SQLite-backed `panel_events` repository.
 *
 * Append-only by design: callers `record(...)` once per panel-level mutation
 * and never update or delete. Reads are admin/offline-job-only — there is no
 * user-facing UI consuming this table.
 */
...
export class SqlitePanelEventRepository implements IPanelEventRepository {
  ...
  async aggregateBySignature(
    ...
    // Build WHERE dynamically. We use IN (...) for eventTypes when present;
    // sqlite-client's sql template only quotes scalars, so build the IN clause
    // by hand via sql.join.
    const conds: ReturnType<typeof sql>[] = [sql`org_id = ${orgId}`];
    conds.push(sql`query_signature IS NOT NULL`);
    if (opts.since) {
    ...
    // Aggregate: count rows per signature; subselect for deletedWithin24h
    // joins deletions back to the earliest 'created' for the same panel_id.
    const rows = this.db.all<{
      signature: string;
      viz_type: string | null;
      count: number;
      deleted_within_24h: number;
    }>(sql`
      SELECT
          ...
          CASE
            WHEN event_type = 'deleted' AND EXISTS (
              ...
              WHERE c.org_id = panel_events.org_id
                ...
                AND (julianday(panel_events.created_at) - julianday(c.created_at)) <= 1.0
            ) THEN 1
            ELSE 0
          END
        ) AS deleted_within_24h
      FROM panel_events
      WHERE ${where}
